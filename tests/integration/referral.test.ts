...
describe('Referral API Integration Tests', () => {
  ...
  async function seedTestData() {
    ...
    testTokens = {
      user1: generateTestToken(user1.id, user1.email),
      user2: generateTestToken(user2.id, user2.email),
    };
  }
  ...
  describe('POST /api/referral/register', () => {
    ...
    it('should register a new user with valid referral code', async () => {
      ...
      expect(response.body.data.user.feeDiscountRate).toBe(0.1); // 10% discount
    });
  ...
  describe('GET /api/referral/network/:userId', () => {
    it('should return user referral network', async () => {
      ...
      expect(response.body.data.children).toHaveLength(1); // user2 is referred by user1
  ...
  describe('GET /api/referral/earnings/:userId', () => {
    beforeEach(async () => {
      // Create sample trade and commission for testing
  ...
  describe('POST /api/referral/claim', () => {
    ...
    it('should reject claim with insufficient balance', async () => {
      const claimData = {
        userId: testUsers.user2.id, // No commissions
        tokenType: 'USDC',
        walletAddress: '0x1234567890abcdef1234567890abcdef12345678',
      };
  ...
  describe('POST /api/webhook/trade', () => {
    it('should process trade and distribute commissions', async () => {
      ...
      expect(commission.earnerId).toBe(testUsers.user1.id); // user1 referred user2
    ...
    it('should validate network-chain combination', async () => {
      const tradeData = {
        ...
        price: '50000',
        chain: 'EVM',
        network: 'Solana', // Invalid: Solana is SVM, not EVM
      };
